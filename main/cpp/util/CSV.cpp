...
const filesystem::path check(const filesystem::path& filename, const filesystem::path& root = filesystem::current_path())
{
...
CSV::CSV(const filesystem::path& path, std::initializer_list<std::string> optLabels) : m_labels(), m_column_count(0)
{
        try {
