...
chrono::year_month_day ConvertFromString(const string& day)
{
        tm           timeinfo{};
        stringstream ss(day);
        ss >> get_time(&timeinfo, "%Y-%m-%d");
        auto date = chrono::year{timeinfo.tm_year + 1900} / chrono::month{static_cast<unsigned int>(timeinfo.tm_mon + 1)} /
                    chrono::day{static_cast<unsigned int>(timeinfo.tm_mday)};
        return date;
}
...
{
        m_day++;
        m_date = static_cast<chrono::year_month_day>(static_cast<chrono::sys_days>(m_date) + chrono::days(1));
}
...
{
        return static_cast<unsigned>(static_cast<chrono::year_month_weekday>(m_date).weekday());
}
